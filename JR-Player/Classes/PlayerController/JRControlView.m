...
@property (nonatomic, strong) UILabel			*totleTime;    //总时间
@property (nonatomic, strong) UIButton			*chooseBtn;    //全凭
...
- (instancetype)initWithFrame:(CGRect)frame {
	...
	if (self = [super initWithFrame:frame]) {
        ...
        self.backBtn =({
            ...
            [backBtn addTarget:self action:@selector(leaveVideoDisplay) forControlEvents:UIControlEventTouchUpInside];
		...
		self.shearBtn = ({
            ...
            [share setTitle:@"1.0X" forState:UIControlStateNormal];
            [share addTarget:self action:@selector(multiplePlayback:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)layoutSubviews {
    ...
    // 6.backBtn
    self.backBtn.frame =CGRectMake(MARGIN, MARGIN, HEAD_H, HEAD_H);
...
-(void)leaveVideoDisplay{
    if ([self.delegate respondsToSelector:@selector(backBtnDidSelected)]) {
        [self.delegate backBtnDidSelected];
    }
}

-(void)multiplePlayback:(UIButton *)sender{
    NSString * rate =[NSString stringWithFormat:@"%0.1f",self.player.player.rate];
    if ([rate isEqualToString:@"1.0"]) {
        [self.shearBtn setTitle:@"1.5X" forState:UIControlStateNormal];
    }else if([rate isEqualToString:@"1.5"]){
        [self.shearBtn setTitle:@"1.8X" forState:UIControlStateNormal];
    }else if([rate isEqualToString:@"1.8"]){
        [self.shearBtn setTitle:@"2.0X" forState:UIControlStateNormal];
    }else if([rate isEqualToString:@"2.0"]){
         [self.shearBtn setTitle:@"1.0X" forState:UIControlStateNormal];
    }
    [self.player playCont];
}
